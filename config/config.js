...
var validateSessionSecret = function (config, testing) {
  ...
  if (config.sessionSecret === 'TasteBuds') {
    if (!testing) {
...
var initGlobalConfigFolders = function (config, assets) {
  ...
  config.folders.client = getGlobbedPaths(path.join(process.cwd(), 'frontend/*/'), process.cwd().replace(new RegExp(/\\/g), '/'));
  config.folders.ng2 = getGlobbedPaths(path.join(process.cwd(), 'node_modules/@angular/*/'), process.cwd().replace(new RegExp(/\\/g), '/'))
    .concat(getGlobbedPaths(path.join(process.cwd(), 'node_modules/angular2-in-memory-web-api/'), process.cwd().replace(new RegExp(/\\/g), '/')))
    .concat(getGlobbedPaths(path.join(process.cwd(), 'node_modules/rxjs/'), process.cwd().replace(new RegExp(/\\/g), '/')));
};
